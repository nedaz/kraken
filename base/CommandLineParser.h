...
commandArg(string name, string descrip) 
  : mName(name), mDesc(descrip),  mHasDefault(false), mHasValue(false) {}

commandArg(string name,string descrip,argType T) 
  : mName(name), mDesc(descrip), mValue(T),  mHasDefault(true), mHasValue(true) {}
...
commandLineParser(int argc, char** argv, const string & description = "") 
  : mArgc(argc), mArgv(argv), mDesc(description), mNumDefaults(0),mNumArgs(0) {}
...
bool GetBoolValueFor(commandArg<bool> &keyArg)
{
  ...
  if ( mIter == mNameVal.end() )
  {
    ...
    if (mIter->second == "0" || mIter->second == "false")
      return false;
    else
      return true;
  //return mIter->second == "1" || mIter->second == "true";
}
