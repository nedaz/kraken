...
int main(int argc,char** argv)
{
  ...
  for (int m = 1; m <3; m++) {
	...
	if (b->getOrient() == (*it)->getCoords().getOrient()) {
      ...
      match.clear();
      annotC.getAnyOverlapping(*it, mode, match);
